This implementation allows atmost 4 clients to connect to the server. After a client's connection request
...
to exit, it can type "exit" as the message.
...
STEP 5: You can open as many clients as you want in separate terminals, but at a time, the server accepts only 4
...
STEP 8: The entered message will be displayed reversed in the T1

STEP 9: After every active client has sent a message to the server or has exited, the server will ask for the 
        broadcast message

STEP 10: After the broadcast message is enterred, it will be displayed reversed in every active client.

STEP 11: After the broadcast message is displayed, the cycle continues and the client asks for a message again.
