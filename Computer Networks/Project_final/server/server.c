  ...
  while (1) {
    ...
    if(strcasecmp(globbuff,"quit") == 0){
      ...
      break;
        ...
        FILE *file = fopen(globbuff, "rb");
        if (file == NULL) {
          struct headerTemp hdr = {htons(0), htonl(0)};
          send(newsock, &hdr, sizeof(struct headerTemp), 0);
        }else {
          struct headerTemp hdr = {htons(1), htonl(0)};
          fseek(file, 0, SEEK_END);
          long bytes_read = ftell(file);
          printf("[+]thread at id %d read: %ld bytes\n", pthread_self(), bytes_read);
          fflush(stdout);
          ...
          while (bytes_read--) {
              ...
              send(newsock, &ch, sizeof(ch), 0);
          }
          fclose(file);
          printf("[+] %s named File sent.\n", globbuff ,pthread_self());
        }
    }
  }
