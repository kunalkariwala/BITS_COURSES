...
class tetParser(Parser):
    ...
    def error(self, p):
        print("Entering Panic Recovery Mode")
        if not p:
            print("End of File!")
            return

        # Read ahead looking for a closing '}' or ')' or the delimiter ';'
        while True:
            tok = next(self.tokens, None)
            if not tok or tok.type == 'R_PAREN' or tok.type == 'R_CURLY' or tok.type == 'SEMICOLON' :
                break
        self.restart()
